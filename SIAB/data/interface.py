...
def _radii_interp(element: str, radius_data: dict):
...
def _van_der_waals_radii(elem: str, kind: str = '0chg'):
    '''
    ...
    data = {'0chg': RVDW_ZERO_CHG, 'eq': RVDW_EQ, 'wiki': RVDW_WIKIPEDIA,
            'crystal': RVDW_CRYSTAL, 'calculated': RVDW_CALCULATED}
    return _radii_interp(elem, data[kind])

def get_radius(elem: str, kind: str = 'covalent', **kwargs):
    ...
    kind : str
        Radius kind, can be 'covalent', 'vdw', 'ionic', 'atomic'.
    ...
    '''
    if kind == 'covalent':
        return _radii_interp(elem, RCOVALENT)
    elif kind == 'vdw':
        return _van_der_waals_radii(elem, kind=kwargs.get('method', '0chg'))
    else:
        raise ValueError('Radius kind %s is not supported.' % kind)

def _unit_convert_ener(from_: str = 'Ha', to_: str = 'eV'):
    '''
    ...
    to_ : str
        the name of unit output
        
    Returns
    ------
    float
        converted data in unit specified by `to_`
    '''
    factors_ = {
