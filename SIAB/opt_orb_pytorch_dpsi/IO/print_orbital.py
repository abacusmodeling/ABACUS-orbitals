...
def print_orbital(orb, info_radial):
	""" orb[it][il][iu][r] """
	for it,orb_t in orb.items():
		#with open(f"orb_{it}.dat","w") as file:
		with open(f"ORBITAL_{periodtable[it]}U.dat","w") as file:
			print_orbital_head(file, orb_t, info_radial, it)
...
def plot_orbital(orb,Rcut,dr):
	for it,orb_t in orb.items():
		#with open(f"orb_{it}_plot.dat","w") as file:
...
def print_orbital_head(file, orb_t, info_radial, it):
	print( "---------------------------------------------------------------------------", file=file )
	print( "Element                     {0}".format(it), file=file )
	print( "Energy Cutoff(Ry)           {0}".format(info_radial["Ecut"][it]), file=file )
	print( "Radius Cutoff(a.u.)         {0}".format(info_radial["Rcut"][it]), file=file )
	print( "Lmax                        {0}".format(len(orb_t)-1), file=file )
	l_name = ["S","P","D"]+list(map(chr,range(ord('F'),ord('Z')+1)))
	for il in range(len(orb_t)):
		print( "Number of {0}orbital-->       {1}".format(l_name[il],len(orb_t[il])), file=file )
	print( "---------------------------------------------------------------------------", file=file )
	print( "SUMMARY  END", file=file )
	print( file=file )
	print( "Mesh                        {0}".format(int(info_radial["Rcut"][it]/info_radial["dr"][it])+1), file=file )
	print( "dr                          {0}".format(info_radial["dr"][it]), file=file )
