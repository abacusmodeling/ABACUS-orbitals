...
def read_input_script(fname, k = None):
    '''read ABACUS INPUT file, decompose it into a dictionary
    
    Parameters
    ----------
    fname : str
        path to the ABACUS INPUT file
    k : str
        key to extract from the dictionary, if None, return the whole dictionary
    
    Returns
    -------
    dict or any
        the dictionary or the value of the key k
    '''
    pairpat = r"^([\w_-]+)(\s+)([^#]*)(.*)$"
...
class _TestDatParse(unittest.TestCase):
    ...
    def test_read_orb_mat(self):
        ...
        fpath = os.path.join(here, 'testfiles/Si/pw/dimer-1.8-gamma/orb_matrix.0.dat')
        ...
        fpath = os.path.join(here, 'testfiles/Si/pw/trimer-1.7-gamma/orb_matrix.1.dat')
    ...
    def test_read_csr(self):
        ...
        import os
        here = os.path.dirname(__file__)
        SR, R = read_csr(os.path.join(here, 'testfiles/data-SR-sparse_SPIN0.csr'))
    ...
    def test_read_wfc_lcao_txt_gamma(self):
        import os
        here = os.path.dirname(__file__)
        wfc, e, occ, k = read_wfc_lcao_txt(os.path.join(here, 'testfiles/WFC_NAO_GAMMA1.txt'))
    ...
    def test_read_wfc_lcao_txt_multik(self):
        import os
        here = os.path.dirname(__file__)
        wfc, e, occ, k = read_wfc_lcao_txt(os.path.join(here, 'testfiles/WFC_NAO_K4.txt'))
    ...
    def test_read_triu_gamma(self):
        import os
        here = os.path.dirname(__file__)
        Tk = read_triu(os.path.join(here, 'testfiles/data-0-T'))
    ...
    def test_read_triu_multik(self):
        ...
        Tk = read_triu(os.path.join(here, 'testfiles/data-2-T'))
        #Tk = read_triu('./testfiles/data-2-T')

        TR, R_all = read_csr(os.path.join(here, 'testfiles/data-TR-sparse_SPIN0.csr'))
    ...
    def test_read_kpoints(self):
        import os
        here = os.path.dirname(__file__)
        k, wk = read_kpoints(os.path.join(here, 'testfiles/kpoints'))
    ...
    def test_read_running_scf_log(self):
        import os
        here = os.path.dirname(__file__)
        dat = read_running_scf_log(os.path.join(here, 'testfiles/running_scf.log1'))
        ...
        dat = read_running_scf_log(os.path.join(here, 'testfiles/running_scf.log2'))
    ...
    def test_read_input_script(self):
        import os
        here = os.path.dirname(__file__)
        dat = read_input_script(os.path.join(here, 'testfiles/INPUT.test'))
        self.assertEqual(dat['calculation'], 'scf')
        self.assertEqual(dat['scf_nmax'], '100')
