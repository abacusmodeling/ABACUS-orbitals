...
def read_abacus_tri(fname):
    ...
    When specifying "out_mat_hs 1" (or out_mat_t 1) in INPUT, ABACUS will
    output H(k) & S(k) (or T(k)) to text files. The content of such files
    merely contains the upper-triangle.
...
class _TestDatParse(unittest.TestCase):
    ...
    def test_read_abacus_tri(self):
        S = read_abacus_tri('./testfiles/data-3-S')
        print(S)

        #T = read_abacus_tri('./testfiles/data-0-T')
