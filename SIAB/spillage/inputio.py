...
diago_full_acc                 0 #all the empty states are diagonalized
...
pseudo_dir                     /home/zuxin/tmp/nao/sg15_oncv_upf_2020-02-06/ #the directory containing pseudo files
...
diago_proc                     8 #the number of procs used to do diagonalization
...
ks_solver                      scalapack_gvx #cg; dav; lapack; genelpa; scalapack_gvx; cusolver
basis_type                     lcao #PW; LCAO in pw; LCAO
use_paw                        0 #whether to use PAW in pw calculation
nbands                         50 #number of bands
...
nocc                           50 #the number of occupied orbitals to form the 2-particle basis ( <= nelec/2)
...
printe                         1 #Print out energy for each band for every printe steps
...
out_mat_hs                     1 8  #output H and S matrix (with precision 8)
out_mat_tk                     1 8  #output T(k)
...
out_wfc_lcao                   1 #ouput LCAO wave functions, 0, no output 1: text, 2: binary
...
bands_to_print                  #specify the bands to be calculated in get_wf and get_pchg calculation
...
qo_strategy                    all  #strategy to generate generate radial orbitals
qo_screening_coeff             0.1  #rescale the shape of radial orbitals
...
def write_input(job_dir, **kwargs):
    ...
    '''
    assert all(key in _valid_keys for key in kwargs), \
            'Invalid INPUT keywords.'

    key_width = max([len(key) for key in kwargs])
...
def read_input(fpath):
    ...
    inputs = dict([re.search(_input_pattern, line).group(1, 3)
                   ...
                   if line.strip() != '' and line.lstrip()[0] != '#' ])
    assert all(key in _valid_keys for key in inputs), 'Invalid INPUT keyword.'
